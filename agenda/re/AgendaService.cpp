...
std::list<Meeting> AgendaService::meetingQuery(const std::string &userName,
	...
	if (!Date::isValid(Date::stringToDate(startDate)) || !Date::isValid(Date::stringToDate(endDate)))
		throw InvaildFormatDateException();
	res=m_storage->queryMeeting([userName,startDate,endDate](const Meeting &c){
		if(c.getSponsor()==userName || c.isParticipator(userName)){
			if(!time_valid(Date::stringToDate(startDate),Date::stringToDate(endDate),c.getStartDate(),c.getEndDate()))
				return true;
		}
		return false;
	});
