...
//private:
  ...
  */
  bool Storage::readFromFile(void){
	//decryptfile(Path::enc_userPath, "123456", Path::dec_userPath);
	//decryptfile(Path::enc_meetingPath, "123456", Path::dec_meetingPath);
    std::ifstream fin;
    fin.open(Path::dec_userPath);
    if(!fin) return false;
    while (fin.good()){
      std::string t_name;
  ...
  */
  bool Storage::writeToFile(void){
    ...
    fout.close();
    fout.open(Path::dec_meetingPath);
    if(!fout) return false;
    flag_n=false;
    ...
    fout.close();
    m_dirty=false;
	//encfile(Path::dec_meetingPath,"123456",Path::enc_meetingPath);
	//encfile(Path::dec_userPath, "123456", Path::enc_userPath);
    return true;
  }
